...
var App = function (makehuman, dat, _, THREE, Detector, Nanobar, Stats) {
    ...
    App.prototype.setModifierDefaults = function () {
        ...
        let modifiers = this.language["Modifiers"];
        let macro_modelling = this.language["Macro modelling"];
        let macro = this.language["Macro"]
        var macroControllers = this.gui.gui.__folders[modifiers].__folders[macro_modelling].__folders[macro].__controllers
        macroControllers.find(c => c.property == this.language["Gender"]).setValue(0)
        macroControllers.find(c => c.property == this.language["Proportions"]).setValue(1)
    }
    ...
    GUI.prototype.setupModifiersGUI = function () {
        ...
        var modifierGui = this.gui.addFolder(this.app.language["Modifiers"]);
        ...
        for (var group in this.app.modeling_sliders) {
            if (this.app.modeling_sliders.hasOwnProperty(group)) {
                ...
                // make the group
                if (!guiGroups[group]) {
                    let groupName = this.app.language[group];
                    var folder = modifierGui.addFolder(groupName);
                    guiGroups[group] = {
                        folder: folder,
                        group: group,
                        subgroups: {}
                ...
                for (var subgroup in groupData) {
                    if (groupData.hasOwnProperty(subgroup)) {
                        ...
                        if (subgroup == "Genitals" || subgroup == "nose features") {
                            continue;
                        }
                        // make subgroups
                        if (!guiGroups[group].subgroups[subgroup]) {
                            let subgroupName = subgroup.charAt(0).toUpperCase() + subgroup.slice(1);
                            subgroupName = this.app.language[subgroupName];
                            var _folder = guiGroups[group].folder.addFolder(subgroupName);
                        ...
                        for (var i = 0; i < subgroupData.length; i++) {
                            ...
                            if (label == "head-angle-in|out" || label.indexOf("Eye bag") != -1 ||
                                label.indexOf("Eye epicanthus") != -1 || label.indexOf("Tone of side chin") != -1 ||
                                label.indexOf("Cleft chin") != -1 || label.indexOf("Nipple") != -1 ||
                                label.indexOf("Navel") != -1) {
                                continue;
                            }

                            label = this.app.language[label]
    ...
    GUI.prototype.setupPoseGUI = function () {
        ...
        this.gui.remember(this.poseConfig);
        this.gui.add(self.poseConfig, '站姿', poseNames).onChange(function (pose) {
    ...
    GUI.prototype.setupSkinGUI = function () {
        ...
        this.gui.add(self.skinConfig, '皮肤', skinNames).onChange(function (skin) {
